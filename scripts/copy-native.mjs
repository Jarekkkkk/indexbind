...
const libraryName = process.platform === 'win32' ? 'inkdex_node.dll' : `libinkdex_node.${extension}`;
const source = path.resolve('target', profile, libraryName);
const targetDir = path.resolve('native');
const target = path.join(targetDir, `inkdex.${process.platform}-${process.arch}.node`);
